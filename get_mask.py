...
def get_mask(path):
    ...
    if image[int(height / 2), int(width - width / 32)].sum() < 60 and image[
        int(height / 2), int(width / 32)].sum() < 60 and image[int(height / 32), int(width / 2)].sum() < 60 and image[
        int(height - height / 32), int(width / 2)].sum() < 60:
        flag = 1
    if flag != 1 and (image[point2, point2].sum() < 450 or image[height - 1 - point2, point2].sum() < 450 or image[
        point2, width - 1 - point2].sum() < 450 or image[height - 1 - point2, width - 1 - point2].sum() < 450):
        flag = 2
